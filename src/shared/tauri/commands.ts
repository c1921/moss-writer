import { invoke } from "@tauri-apps/api/core";

import type { DirectoryEntry, FileEntry, ProjectSnapshot } from "../../app/types";
import type { WebDavSettings } from "../../features/settings/types";
import type { SyncResolveStrategy, SyncResponse } from "../../features/sync/types";

function command<T>(name: string, args?: Record<string, unknown>) {
  return invoke<T>(name, args);
}

export function openProject(directory: string) {
  return command<ProjectSnapshot>("open_project", { directory });
}

export function readFile(path: string) {
  return command<string>("read_file", { path });
}

export function writeFile(path: string, content: string) {
  return command<void>("write_file", { path, content });
}

export function listFiles(directory: string) {
  return command<FileEntry[]>("list_files", { directory });
}

export function listDirectories(directory: string) {
  return command<DirectoryEntry[]>("list_directories", { directory });
}

export function createFile(path: string) {
  return command<FileEntry>("create_file", { path });
}

export function createDirectory(path: string) {
  return command<void>("create_directory", { path });
}

export function renameFile(path: string, newName: string) {
  return command<FileEntry>("rename_file", { path, newName });
}

export function deleteFile(path: string) {
  return command<void>("delete_file", { path });
}

export function renameDirectory(path: string, newName: string) {
  return command<void>("rename_directory", { path, newName });
}

export function deleteDirectory(path: string) {
  return command<void>("delete_directory", { path });
}

export function getSyncSettings() {
  return command<WebDavSettings>("get_sync_settings");
}

export function saveSyncSettings(settings: WebDavSettings) {
  return command<WebDavSettings>("save_sync_settings", { settings });
}

export function testSyncConnection(settings: WebDavSettings) {
  return command<SyncResponse>("test_sync_connection", { settings });
}

export function syncPush() {
  return command<SyncResponse>("sync_push");
}

export function syncPull() {
  return command<SyncResponse>("sync_pull");
}

export function resolveSyncPending(strategy: SyncResolveStrategy) {
  return command<SyncResponse>("resolve_sync_pending", { strategy });
}
