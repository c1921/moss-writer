...
export function AppearancePanel({ settings, onChangeSettings }: AppearancePanelProps) {
  function updateNumber(key: "miniWindowOpacity", rawValue: string, min: number, max: number) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <Label className="shrink-0 text-sm">小窗背景不透明度</Label>
          <div className="flex items-center gap-1.5">
            <Input
              className="w-20 text-right"
              max={100}
              min={10}
              onChange={(event) => updateNumber("miniWindowOpacity", event.currentTarget.value, 10, 100)}
              step={5}
              type="number"
